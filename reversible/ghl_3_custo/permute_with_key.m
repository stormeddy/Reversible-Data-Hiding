function [JI]=permute_with_key(I,permutation_key,u,v)
    % JI: image after permutation  
    [m,n]=size(I);
    
    
    % scramble using permutation key
    hor_num=floor(m/u);
    ver_num=floor(n/v);
    
    f=zeros(hor_num,ver_num);
    S=ksa(permutation_key);
    KSTREAM=prga_crypt(f,S);
    [~,index]=sort(KSTREAM(:));   
    
    
    B=1:hor_num*ver_num;
    Bp=B(index);
    
    JI=I;
    len_key=length(permutation_key);
    
    for i=1:hor_num*ver_num
       ind=Bp(i);
       [x,y] = ind2sub([hor_num,ver_num],ind);
       [xi,yi] = ind2sub([hor_num,ver_num],i);
       tmp=I((x-1)*u+1:x*u,(y-1)*v+1:y*v);
       % permute within differendt blocks with different keys
       % different keys are generated by strcat(...)
       ptmp=permute_within_block(tmp,strcat(permutation_key,permutation_key(mod(ind,len_key)+1)));
       JI((xi-1)*u+1:xi*u,(yi-1)*v+1:yi*v)=ptmp;
    end
end