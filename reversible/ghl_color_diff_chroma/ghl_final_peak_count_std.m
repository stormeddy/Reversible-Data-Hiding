...
riwuit_non_sample_peak_count_std=zeros(Num,3);
ghl_non_sample_peak_count_std=zeros(Num,3);
...
for i=1:Num
    ...
    for j=1:3
        ...
        riwuit_non_sample_peak_count_std(i,j)=cal_peak_count(Ch3);
    ...
    for j=1:3
        ...
        % record corresponding peak count of non_sample interpolation
        ghl_non_sample_peak_count_std(i,j)=cal_peak_count(Ch3,Ch1,Ch2,N);
...
diff_non_sample_peak_count_std=ghl_non_sample_peak_count_std-riwuit_non_sample_peak_count_std;
[max_diff_non_sample_peak_count_std,flag_peak_count_std]=max(diff_non_sample_peak_count_std,[],2);
flag_peak_count_std(max_diff_non_sample_peak_count_std<=0)=0;

all_ratio_final_std=sum(ratio_final_std,2);
reversibility_final_std=bitand(img_equal_final_std,bit_equal_final_std);

save_path='which_channel_peak_count_std\';
...
save([save_path 'riwuit_non_sample_peak_count_std.mat'],'riwuit_non_sample_peak_count_std');
save([save_path 'ghl_non_sample_peak_count_std.mat'],'ghl_non_sample_peak_count_std');
save([save_path 'flag_peak_count_std.mat'],'flag_peak_count_std');